"""Index/grouping helpers for static STYLE Pyomo model construction."""
...
def vhp_sources_by_vhp(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
        vhp.name: tuple(
...
def boilers_by_vhp(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
        vhp.name: tuple(
...
def hrsgs_by_vhp(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
...
def flash_level_by_name(data: StyleModelData) -> dict[str, FlashSteamRecoveryLevel]:
    if data.flash_steam_recovery is None:
        return {}
    return {level.steam_level: level for level in data.flash_steam_recovery.levels}


def flash_route_by_name(data: StyleModelData) -> dict[str, FlashSteamRecoveryRoute]:
    if data.flash_steam_recovery is None:
        return {}
    return {route.name: route for route in data.flash_steam_recovery.routes}


def flash_routes_by_source(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    if data.flash_steam_recovery is None:
...
def flash_routes_by_target(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    if data.flash_steam_recovery is None:
        return {level.name: () for level in data.steam_levels}
    return {
        level.name: tuple(
...
def steam_main_turbines_by_source(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
        level.name: tuple(
            turbine.name
...
def steam_main_turbines_by_target(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
        level.name: tuple(
            turbine.name
            for turbine in data.steam_main_turbines
...
def steam_main_letdowns_by_source(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
        level.name: tuple(
            letdown.name
...
def steam_main_letdowns_by_target(data: StyleModelData) -> dict[str, tuple[str, ...]]:
    return {
        level.name: tuple(
            letdown.name
            for letdown in data.steam_main_letdowns
