"""Result extraction and benchmark comparison helpers for utility-system models."""
...
@dataclass(frozen=True)
class UtilitySystemFuelConsumptionFamily:
...
@dataclass(frozen=True)
class UtilitySystemFuelConsumptionEquipment:
...
@dataclass(frozen=True)
class UtilitySystemFuelCapacityContext:
...
@dataclass(frozen=True)
class UtilitySystemOperatingCostComponent:
...
@dataclass(frozen=True)
class UtilitySystemBenchmarkDeviation:
...
class UtilitySystemBenchmarkRecord(Protocol):
    @property
...
@dataclass(frozen=True)
class UtilitySystemBenchmarkComparison:
    ...
    def deviation_for(self, field: str) -> UtilitySystemBenchmarkDeviation:
...
@dataclass(frozen=True)
class UtilitySystemBestConfigurationComparison:
...
def extract_utility_system_result(
...
def utility_system_fuel_consumption_by_family(
    model: pyo.ConcreteModel,
...
def utility_system_operating_cost_components(
    model: pyo.ConcreteModel,
    ...
    return (
        UtilitySystemOperatingCostComponent("fuel", fuel),
        UtilitySystemOperatingCostComponent("hot_oil", hot_oil),
        UtilitySystemOperatingCostComponent("electricity", electricity),
        UtilitySystemOperatingCostComponent("auxiliary_or_unallocated", auxiliary),
        UtilitySystemOperatingCostComponent("total", total),
    )


def compare_utility_system_result_to_benchmark(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "result and benchmark identify different utility-system scenarios"
        )
    fields = (
        "utility_steam_flow",
        "fuel_consumption",
    ...
    deviations: list[UtilitySystemBenchmarkDeviation] = []
    for field in fields:
    ...
    return UtilitySystemBenchmarkComparison(
        actual=result,
        benchmark=benchmark,
...
def compare_utility_system_result_to_best_configuration(
    result: UtilitySystemResult,
    benchmark: BestConfigurationBenchmarkRecord,
    *,
    absolute_tolerance: float = 1e-6,
    ...
    return UtilitySystemBestConfigurationComparison(
        actual=result,
        benchmark=benchmark,
...
def _benchmark_deviation(
    ...
    return UtilitySystemBenchmarkDeviation(
        field=field,
        actual=actual,
        benchmark=benchmark,
...
def _benchmark_value(
    benchmark: UtilitySystemBenchmarkRecord, field: str
) -> float | None:
...
def _indexed_product_fuel_consumption_equipment_row(
    ...
    *,
    ...
    return UtilitySystemFuelConsumptionEquipment(
        equipment_family=equipment_family,
        equipment_name=_format_index(index),
        fuel_variable=_component_label(variable_name, index),
...
def _indexed_product_fuel_capacity_context_row(
    ...
    *,
    ...
    return UtilitySystemFuelCapacityContext(
        equipment_family=equipment_family,
        equipment_name=_format_index(index),
...
def _hot_oil_fuel_capacity_context_row(
    model: pyo.ConcreteModel,
) -> UtilitySystemFuelCapacityContext | None:
...
def _scalar_fuel_consumption_equipment(
    ...
    *,
    ...
    factor_name: str,
    included_in_table_fuel_consumption: bool,
) -> UtilitySystemFuelConsumptionEquipment | None:
    if not hasattr(model, variable_name):
        return None
    return UtilitySystemFuelConsumptionEquipment(
        equipment_family=equipment_family,
