"""Solver-independent helpers for utility-system bilevel decomposition bookkeeping."""
...
from .results import extract_utility_system_result
from .runner import (
...
UTILITY_SYSTEM_MASTER_BINARY_COMPONENT_NAMES: tuple[str, ...] = (
    "level_selected",
    "hot_oil_selected",
...
def utility_system_master_binary_variables(
    model: pyo.Block,
    *,
    component_names: Iterable[str] = UTILITY_SYSTEM_MASTER_BINARY_COMPONENT_NAMES,
) -> dict[str, Any]:
    ...
    for component_name in component_names:
        ...
        if component is None or component.ctype is not pyo.Var:
            continue
        variables.update(
            _utility_system_binary_variables_from_component(component_name, component)
        )
    return variables


def utility_system_master_integer_assignment_from_model(
    model: pyo.Block,
    *,
    threshold: float = 0.5,
    component_names: Iterable[str] = UTILITY_SYSTEM_MASTER_BINARY_COMPONENT_NAMES,
    ...
    variables = utility_system_master_binary_variables(
        model,
        component_names=component_names,
    )
    if not variables:
        raise ValueError("utility-system model contains no master binary variables")
    values: dict[str, int] = {}
...
def fix_utility_system_master_integer_assignment(
    model: pyo.Block,
    assignment: BilevelIntegerAssignment,
    *,
    component_names: Iterable[str] = UTILITY_SYSTEM_MASTER_BINARY_COMPONENT_NAMES,
    ...
    if missing_values:
        raise KeyError(
            f"missing utility-system master binary values in assignment: {missing_values!r}",
        )
    unknown_values = tuple(name for name in values if name not in variables)
    if unknown_values:
        raise KeyError(
            f"unknown utility-system master binary values in assignment: {unknown_values!r}",
        )
    for name, variable in variables.items():
...
def utility_system_fixed_assignment_subproblem_result(
    ...
    *,
    ...
    fix_utility_system_master_integer_assignment(model, assignment)
    solver_status = _normalize_utility_system_solver_status(solve(model))
    _raise_if_failed_utility_system_subproblem(solver_status)
    ...
    return BilevelSubproblemResult(
        ...
        status=_utility_system_subproblem_status_label(solver_status),
        source_method="utility-system-fixed-assignment",
    )


def run_utility_system_fixed_assignment_decomposition(
    scenario: UtilitySystemScenario,
    *,
    solve_master: UtilitySystemSolve,
    solve_subproblem: UtilitySystemSolve,
    max_iterations: int,
    absolute_gap_tolerance: float | None = None,
) -> BilevelDecompositionRun:
    ...
    return run_bilevel_decomposition(
        lambda: build_utility_system_model(scenario.data),
        solve_master=solve_master,
        solve_subproblem=lambda assignment: (
        ...
        binary_variables=utility_system_master_binary_variables,
        assignment_from_model=utility_system_master_integer_assignment_from_model,
    )


def build_utility_system_binary_selection_master(
    ...
    variable_names = tuple(utility_system_master_binary_variables(full_model))
    if not variable_names:
        raise ValueError("utility-system data produced no master binary variables")
    model = pyo.ConcreteModel(name="static utility-system binary selection master")
...
def utility_system_binary_selection_master_assignment_from_model(
    model: pyo.Block,
    *,
    threshold: float = 0.5,
) -> BilevelIntegerAssignment:
...
def utility_system_binary_selection_candidate_solver(
...
def utility_system_binary_selection_candidate_from_scenario(
    scenario: UtilitySystemScenario,
    *,
    solve: UtilitySystemSolve,
) -> BilevelIntegerAssignment:
    ...
    return utility_system_master_integer_assignment_from_model(run.model)


def utility_system_binary_selection_candidates_from_scenarios(
    ...
    *,
    ...
    return tuple(
        record.assignment
        for record in utility_system_binary_selection_candidate_records_from_scenarios(
            scenarios,
            solve=solve,
        )
    )
...
def utility_system_binary_selection_candidate_records_from_scenarios(
    scenarios: Iterable[UtilitySystemScenario],
    *,
    solve: UtilitySystemSolve,
    source_label_factory: Callable[[UtilitySystemScenario], str] | None = None,
    ...
    for scenario in scenarios:
        candidate = utility_system_binary_selection_candidate_from_scenario(
            scenario,
            solve=solve,
        )
        ...
        records.append(
            BilevelCandidateAssignment(
                ...
                source_label=(
                    _utility_system_scenario_source_label(scenario)
...
def run_utility_system_binary_selection_decomposition(
    scenario: UtilitySystemScenario,
    *,
    solve_master: Callable[[pyo.Block], Any],
    solve_subproblem: UtilitySystemSolve,
    ...
    return run_bilevel_decomposition(
        lambda: build_utility_system_binary_selection_master(scenario.data),
        solve_master=solve_master,
        solve_subproblem=lambda assignment: (
            utility_system_fixed_assignment_subproblem_result(
                scenario,
        ...
        assignment_from_model=utility_system_binary_selection_master_assignment_from_model,
    )


def run_utility_system_binary_selection_candidate_decomposition(
    scenario: UtilitySystemScenario,
    *,
    ...
    solve_master = utility_system_binary_selection_candidate_solver(
    ...
    while len(iterations) < max_iterations:
        try:
            master_model = _build_utility_system_binary_selection_master_with_cuts(
                scenario.data,
            ...
            break
        assignment = utility_system_binary_selection_master_assignment_from_model(
            master_model
        )
        try:
        ...
        next_master_model = _build_utility_system_binary_selection_master_with_cuts(
            scenario.data,
            solution_pool=solution_pool,
            failed_cuts=failed_cuts,
        )
...
def _build_utility_system_binary_selection_master_with_cuts(
    data: UtilitySystemModelData,
    *,
    solution_pool: BilevelSolutionPool,
    failed_cuts: Iterable[BilevelIntegerCut],
) -> pyo.ConcreteModel:
    model = build_utility_system_binary_selection_master(data)
...
def _utility_system_scenario_source_label(scenario: UtilitySystemScenario) -> str:
...
def _resolve_binary_variables(model: pyo.Block, binary_variables: Any) -> Any:
    if binary_variables is None:
        return utility_system_master_binary_variables(model)
...
def _normalize_utility_system_solver_status(
...
def _raise_if_failed_utility_system_subproblem(
    status: UtilitySystemSolverStatus,
) -> None:
    termination = (
        None
    ...
    if termination not in {"error", "infeasible", "limit", "unbounded"}:
        return
    raise RuntimeError(
        "Static utility-system fixed-assignment subproblem did not produce an "
        f"extractable solution: {status}",
    )


def _utility_system_subproblem_status_label(
    status: UtilitySystemSolverStatus,
) -> str:
...
def _utility_system_binary_variables_from_component(
    component_name: str,
    component: Any,
) -> dict[str, Any]:
    if component.is_indexed():
        return {
            _utility_system_master_variable_name(component_name, index): component[
                index
            ]
...
def _utility_system_master_variable_name(component_name: str, index: Any) -> str:
