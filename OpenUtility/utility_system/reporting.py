...
def utility_system_decomposition_trajectory_rows(
    *,
    catalog: str,
...
def format_utility_system_decomposition_trajectory_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system decomposition trajectory output format {output_format!r}"
    )


def utility_system_decomposition_skipped_candidate_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    run: BilevelDecompositionRun,
...
def utility_system_candidate_pool_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_candidate_pool_comparison_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_candidate_selection_delta_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_candidate_selection_delta_summary_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_candidate_source_filter_summary_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_candidate_source_filter_detail_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_candidate_source_filter_variable_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def utility_system_skipped_candidate_delta_summary_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    run: BilevelDecompositionRun,
...
def utility_system_candidate_audit_bundle_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    candidates: Iterable[BilevelCandidateAssignment],
...
def format_utility_system_candidate_audit_bundle_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate audit-bundle output format {output_format!r}"
    )


def format_utility_system_skipped_candidate_delta_summary_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system skipped-candidate delta summary output format "
        f"{output_format!r}"
    )


def format_utility_system_candidate_source_filter_summary_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate source-filter summary output format "
        f"{output_format!r}"
    )


def format_utility_system_candidate_source_filter_detail_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate source-filter detail output format "
        f"{output_format!r}"
    )


def format_utility_system_candidate_source_filter_variable_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate source-filter variable output format "
        f"{output_format!r}"
    )


def format_utility_system_candidate_selection_delta_summary_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate-selection summary output format {output_format!r}"
    )


def format_utility_system_candidate_selection_delta_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate-selection delta output format {output_format!r}"
    )


def format_utility_system_candidate_pool_comparison_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate-pool comparison output format {output_format!r}"
    )


def format_utility_system_candidate_pool_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "csv":
        return _format_rows_csv(
            materialized_rows, UTILITY_SYSTEM_CANDIDATE_POOL_ROW_FIELDS
        )
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system candidate-pool output format {output_format!r}"
    )


def format_utility_system_decomposition_skipped_candidate_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system skipped-candidate output format {output_format!r}"
    )


def utility_system_decomposition_objective_comparison_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    run: BilevelDecompositionRun,
    ...
    return tuple(
        _utility_system_decomposition_objective_comparison_row(
            catalog=catalog,
            scenario=scenario,
...
def format_utility_system_decomposition_objective_comparison_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        "unsupported utility-system decomposition objective comparison output format "
        f"{output_format!r}"
    )


def utility_system_fuel_consumption_family_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    model: Any,
    ...
    family_rows = utility_system_fuel_consumption_by_family(model)
...
def format_utility_system_fuel_consumption_family_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system fuel-family output format {output_format!r}"
    )


def utility_system_fuel_consumption_equipment_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    model: Any,
    ...
    return tuple(
        ...
        for row in utility_system_fuel_consumption_by_equipment(model)
    )


def format_utility_system_fuel_consumption_equipment_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system fuel-equipment output format {output_format!r}"
    )


def utility_system_fuel_consumption_capacity_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    model: Any,
    ...
    equipment_rows = utility_system_fuel_consumption_equipment_rows(
        catalog=catalog,
        scenario=scenario,
        model=model,
        benchmark=benchmark,
    ...
    capacity_context = {
        ...
        for row in utility_system_fuel_capacity_context_by_equipment(model)
    }
    rows: list[dict[str, Any]] = []
    for row in equipment_rows:
        ...
        if context is None:
            continue
        rows.append(_utility_system_fuel_consumption_capacity_row(row, context))
    return tuple(rows)


def format_utility_system_fuel_consumption_capacity_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system fuel-capacity output format {output_format!r}"
    )


def utility_system_fuel_consumption_diagnosis_rows(
    rows: Iterable[dict[str, Any]],
) -> tuple[dict[str, Any], ...]:
    ...
    diagnosis_rows = [
        _utility_system_fuel_consumption_diagnosis_row(group_rows)
...
def format_utility_system_fuel_consumption_diagnosis_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system fuel-diagnosis output format {output_format!r}"
    )


def utility_system_fuel_calibration_target_rows(
    rows: Iterable[dict[str, Any]],
) -> tuple[dict[str, Any], ...]:
    ...
    target_rows = [
        _utility_system_fuel_calibration_target_row(group_rows)
        for group_rows in grouped_rows.values()
    ]
    return tuple(
        {
...
def format_utility_system_fuel_calibration_target_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system fuel-target output format {output_format!r}"
    )


def utility_system_fuel_consumption_factor_map_from_calibration_target_rows(
    rows: Iterable[dict[str, Any]],
...
def utility_system_operating_cost_component_rows(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    model: Any,
    ...
    return tuple(
        ...
        for row in utility_system_operating_cost_components(model)
    )


def format_utility_system_operating_cost_component_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system operating-components output format {output_format!r}"
    )


def utility_system_operating_cost_target_rows(
    rows: Iterable[dict[str, Any]],
    *,
...
def format_utility_system_operating_cost_target_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system operating-targets output format {output_format!r}"
    )


def utility_system_operating_cost_adjustment_map_from_target_rows(
    rows: Iterable[dict[str, Any]],
...
def utility_system_fuel_consumption_residual_ranking_rows(
    rows: Iterable[dict[str, Any]],
) -> tuple[dict[str, Any], ...]:
    ...
    ranking_rows = [
        _utility_system_fuel_consumption_residual_ranking_row(group_rows)
        for group_rows in grouped_rows.values()
    ]
    return tuple(
        {
...
def format_utility_system_fuel_consumption_residual_ranking_rows(
    rows: Iterable[dict[str, Any]],
    *,
    output_format: str,
) -> str:
    ...
    if output_format == "json":
        return json.dumps(materialized_rows, indent=2)
    raise ValueError(
        f"unsupported utility-system fuel-residual ranking output format {output_format!r}"
    )
...
def _utility_system_decomposition_objective_comparison_row(
    *,
    catalog: str,
    scenario: UtilitySystemScenario,
    iteration_index: int,
...
def _utility_system_fuel_consumption_residual_ranking_row(
...
def _utility_system_fuel_consumption_capacity_row(
    row: dict[str, Any],
    context: Any,
) -> dict[str, Any]:
    return {
...
def _utility_system_fuel_consumption_diagnosis_row(
    rows: Sequence[dict[str, Any]],
) -> dict[str, Any]:
    scenario_row = rows[0]
...
def _utility_system_fuel_calibration_target_row(
    rows: Sequence[dict[str, Any]],
) -> dict[str, Any]:
    scenario_row = rows[0]
